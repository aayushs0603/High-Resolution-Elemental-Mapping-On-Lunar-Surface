...
def tryExceptProcess_subgraph(df, subregion_row, subregion_col, lat_start, lon_start, lat_size, lon_size, itr_no):
    try:
        process_subgraph(df, subregion_row, subregion_col, lat_start, lon_start, lat_size, lon_size, itr_no)
    except:
        print(f"[ERROR] {current_time()} Failed graph generation for {subregion_row} {subregion_col} {lat_start} {lon_start}")
    ...
    with ThreadPoolExecutor() as executor:
        for j_index, lon_start in enumerate(lon_positions):
            ...
            for i_index, lat_start in enumerate(lat_positions):
                ...
                futures.append(executor.submit(tryExceptProcess_subgraph, df, subregion_row, subregion_col, lat_start, lon_start, lat_sizes[i_index], lon_sizes[j_index], iteration_number))
